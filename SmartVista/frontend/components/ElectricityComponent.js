import React from 'react'
import elec from '../data/electiricty'
import Link from 'next/link'
const Electricity = () => {
    return (
        <>
            <div className='border border-black px-20 rounded-xl ml-16 mr-10 bg-blue-300 flex justify-center items-center mt-10'>
                <div className="flex flex-row">
                    <div className='flex justify-center w-1/2'>
                        <img className='w-96 h-64' src='/mainelec.jpg' alt='' />
                    </div>
                    <div className='px-10 flex flex-col justify-center items-center w-1/2'>
                        <div className='h-1/2'>
                            <h1 className='text-8xl font-mono'>
                                Electricity
                            </h1>
                        </div>
                        <div className='h-1/2'>
                            <h1 className='text-sm font-mono underline'>
                                Imagine a city that never sleeps, a vibrant constellation of dreams powered by an unseen force. That force is Delhi Electricity, the lifeblood of this bustling metropolis. From the hum of refrigerators in humble homes to the neon glow of skyscrapers, they fuel Delhi's aspirations, one watt at a time.


                            </h1>
                        </div>
                    </div>
                </div>
            </div>

            <div className='border border-black p-10 mt-16 mr-16 ml-16 rounded-xl bg-blue-300 flex flex-col justify-center'>
                <div className='flex justify-center h-1/2'>
                    <h1 className='text-8xl font-mono'>
                        Electricity
                    </h1>
                </div>
                <div className='h-1/2 mt-5'>
                    <h1 className='text-sm font-mono '>
                        But Delhi Electricity is more than just wires and transformers. They're innovators, constantly seeking sustainable solutions like solar power and grid modernization. They're champions of accessibility, ensuring even the remotest corners are illuminated. And they're committed to a cleaner future, reducing carbon footprint and powering Delhi's green ambitions.

                    </h1>
                </div>
            </div>

            <div className='flex justify-center items-center mt-16 font-mono'>
                <h1 className='text-5xl'>Delhi Electricity Service</h1>
            </div>
            <div className="px-20 mt-10 mb-10">

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
                    {elec.map((item) => {
                        return (
                            <>
                                <div className="flex justify-center flex-col space-y-3 cursor-pointer">
                                    <div className="relative h-auto w-auto group">
                                        <img className="h-48 w-48 rounded-xl object-cover" src={item.imageURL} />
                                    </div>
                                    <div>
                                        <div className="flex justify-between items-center mt-5">
                                            <h3 className="text-2xl">
                                                {item.name}
                                            </h3>
                                            <div className="flex items-center space-x-1">

                                            </div>
                                        </div>
                                        <Link href={item.link}> 
                                            <button className="mt-3 px-3 text-lg bg-yellow-300  hover:bg-blue-500 text-black hover:text-white  border border-black hover:border-transparent rounded">
                                                Checkout
                                            </button>
                                        </Link>
                                    </div>
                                </div>
                            </>
                        )
                    })}

                </div>
            </div>
        </>
    )
}

export default Electricity